...
from integration import gen_descent_Lag, lin_zigzag
...
print('Getting true path of least action')
...
tilde_x0= lin_zigzag.sol_lin_gen_coord(F,x,t,x0,np.zeros([dim,order,N]))[:,:,0] #last square bracket is to get first sample only
genx=gen_coords.serial_derivs_xt(x,t,order+1)

# scaling of lagrangian (in log_10 scale)
scaling=range(3)

# Solution
xt_gen_gd_Lag=np.zeros([len(scaling),dim,order+1,Time.size])
# yt_gen_gd_Lag=np.zeros([len(scaling),dim,order+1,Time.size])

# Lagrangian of solution
Lag_xt_gen_gd=np.zeros([len(scaling),Time.size])
# Lag_yt_gen_gd=np.zeros([len(scaling),Time.size])
...
for s in scaling:
    ...
    'integrate path of least action Euler'
    xt_gen_gd_Lag[s,:,:,:]=gen_descent_Lag.Euler_gen_flow(tilde_x0,flow_gen_gd_Lag,genx,Time)

    # 'integrate paths of least action RK45'
    # yt_gen_gd_Lag,Time_rk45=gen_descent_Lag.RK45_gen_flow(tilde_x0,flow_gen_gd_Lag,genx,Time)

    'Lagrangian over time'
    print('Lagrangian over time')
    for tau in range(Time.size):
        if tau % 10**5 == 0:
            print(tau/Time.size)
        Lag_xt_gen_gd[s,tau]= Lag_lambdify(*xt_gen_gd_Lag[s,:,:,tau].ravel())
        # Lag_yt_gen_gd[s, tau] = Lag_lambdify(*yt_gen_gd_Lag[s, :, :, tau].ravel())


'''Part 6: Plots'''

plot_timesteps=timesteps#min(280,Time.size)
plot_indices=range(min(int(plot_timesteps),timesteps))
lw=0.5
alpha=0.3

'Part 6a: 2D plot of linear Zig-zag method sample paths'

#n=1 #trajectory to plot
plt.figure(0)
plt.clf()
plt.suptitle(f'Lagrangian', fontsize=16)
plt.title(r'$t \mapsto\mathcal{L} (x^{(:N+1)}_t)+1$', fontsize=13)
# plt.title(f'order={order},T={T},steps/time={int(timesteps/T)}', fontsize=14)
for s in scaling: #range(len(scaling)):
    plt.plot(Time[plot_indices], Lag_xt_gen_gd[s,plot_indices]+1,alpha=alpha)
plt.yscale('log')
# plt.ylabel(r'$\mathcal{L} (x^{:N+1}_t)+1$')
plt.xlabel(r'Time $t$')
# plt.legend()
plt.savefig("OU_2D_least_Lag_t.png", dpi=100)


plt.figure(1)
plt.clf()
plt.suptitle(f'Least action', fontsize=16)
plt.title(r'$\dot{x}^{(:N+1)}_t = \mathbf{D}_N x^{(:N+1)}_t-\lambda\nabla\mathcal{L}( x^{(:N+1)}_t)$', fontsize=13)
# plt.title(f'order={order},T={T},steps/Time={int(timesteps/T)}', fontsize=14)
plt.plot(xt_least[0,plot_indices,0], xt_least[1,plot_indices,0],linestyle=':', color='gray',label=f'true')
for s in scaling:
    plt.plot(xt_gen_gd_Lag[s,0,0,plot_indices], xt_gen_gd_Lag[s,1,0,plot_indices],label=rf'$\lambda=${10**s} ',alpha=alpha)
plt.legend()
# plt.ylabel(r'${x}_t$')
# plt.xlabel(r'Time $t$')
plt.savefig("OU_2D_least_xt.png", dpi=100)
...
# plt.suptitle(f'Least action simulations Euler vs RK45', fontsize=16)
# plt.title(f'order={order},T={T},steps/Time={int(timesteps/T)}', fontsize=14)
# plt.plot(xt_least[0,plot_indices,0], xt_least[1,plot_indices,0], color='black',label=f'true')
# plt.plot(xt_gen_gd_Lag[s,0,0,plot_indices], xt_gen_gd_Lag[s,1,0,plot_indices],linestyle=':',label=f'Euler')
# plt.plot(yt_gen_gd_Lag[0,0,:], yt_gen_gd_Lag[1,0,:],linestyle=':',label=f'RK45')
# plt.legend()
